...
export default function EditUniversityBuildingDialog({ open, row, onClose, onSave }) {
  ...
  useEffect(() => {
    if (!open || !row) return;
    setName(row.name ?? '');
    setEntityCount(String(row.entityCount ?? 0));
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 id="edit-building-title" className="text-lg font-semibold text-[#101828]">
              Edit building
            </h2>
            <p className="mt-1 text-sm text-[#667085]">Update details for this building.</p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#344054]" htmlFor="uni-bdg-edit-name">
              Building name
            </label>
            <input
              id="uni-bdg-edit-name"
              value={name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-[#344054]" htmlFor="uni-bdg-edit-count">
                Count
              </label>
              <input
                id="uni-bdg-edit-count"
                value={entityCount}
            ...
            <div>
              <label className="block text-sm font-medium text-[#344054]" htmlFor="uni-bdg-edit-used">
                Used
              </label>
              <input
                id="uni-bdg-edit-used"
                value={quotaUsed}
          ...
          <div>
            <label className="block text-sm font-medium text-[#344054]" htmlFor="uni-bdg-edit-total">
              Total
            </label>
            <input
              id="uni-bdg-edit-total"
              value={quotaTotal}
        ...
        <div className="mt-7 flex items-center justify-end gap-3 border-t border-[#F2F4F7] pt-5">
          ...
          <button
            ...
            onClick={() => {
              ...
              onSave?.({
                name: trimmed,
                entityCount: parseIntSafe(entityCount),
                quotaUsed: parseIntSafe(quotaUsed),
                quotaTotal: Math.max(1, parseIntSafe(quotaTotal)),
              });
            }}
          >
            Save
