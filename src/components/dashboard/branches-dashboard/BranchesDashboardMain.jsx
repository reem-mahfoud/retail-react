...
import { useBranchesDashboard } from 'hooks/useBranchesDashboard';
import AddBranchDialog from './AddBranchDialog';
import EditBranchDialog from './EditBranchDialog';
import BranchRowCard from './BranchRowCard';

/**
 * Branches screen: white page (breadcrumb, title, add), card body (tabs, search, back strip, list).
 */
export default function BranchesDashboardMain({ className = '' }) {
  const {
    listStatus,
    setListStatus,
    search,
    ...
    <div className={`flex w-full max-w-none flex-col bg-white pb-10 font-sans antialiased ${className}`.trim()}>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-normal leading-5 text-[#535862] sm:text-[15px] sm:leading-6">
              Building creation and management page.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="mx-6 mb-8 flex flex-col rounded-2xl border border-[#E9EAEB] bg-white shadow-[0px_1px_2px_rgba(16,24,40,0.05)] sm:mx-8">
        ...
        <div className="border-b border-[#E9EAEB] bg-[#FAFAFA] px-5 py-2.5 sm:px-6">
          ...
          >
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-[#E9EAEB] bg-white text-sm text-[#717680] shadow-sm" aria-hidden>
              ‹
            </span>
            Organizations
          </Link>
      ...
      <AddBranchDialog open={addOpen} onClose={closeAdd} onSubmit={addBranch} />
