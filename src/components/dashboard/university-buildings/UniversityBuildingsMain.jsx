...
import { useUniversityBuildingsDashboard } from 'hooks/useUniversityBuildingsDashboard';
import AddBuildingDialog from './AddBuildingDialog';
import EditUniversityBuildingDialog from './EditUniversityBuildingDialog';
import UniversityBuildingRow from './UniversityBuildingRow';

export default function UniversityBuildingsMain() {
  ...
  const {
    ...
    paginationItems,
    universityIdInvalid,
  } = useUniversityBuildingsDashboard(orgId);

  return (
    <div className={`${CRADLE_PAGE_COLUMN} font-sans antialiased`}>
      <header className={CRADLE_PAGE_HEADER}>
        <DashboardPageBreadcrumb variant="universities-building" label={universityName} />
        <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-1">
            <h1 className="text-2xl font-semibold leading-8 tracking-tight text-[#181D27]">
              Buildings of {universityName}
            </h1>
            <p className="text-base font-normal leading-6 text-[#535862]">Page for creating and managing buildings.</p>
          </div>
          <button
            type="button"
      ...
      <div className={CRADLE_TABLE_CARD}>
        ...
        <ListQueryPanel
          ...
          emptyState={
            universityIdInvalid ? (
              <ListEmptyState>Invalid university id in URL.</ListEmptyState>
            ) : (
              <ListEmptyState>No buildings found.</ListEmptyState>
            )
          }
      ...
      <button type="button" className="sr-only" onClick={() => navigate('/dashboard/organizations')}>
        Back
      </button>

      <EditUniversityBuildingDialog
      ...
      <AddBuildingDialog open={addOpen} onClose={closeAdd} onSubmit={submitAdd} />
    </div>
  );
}
